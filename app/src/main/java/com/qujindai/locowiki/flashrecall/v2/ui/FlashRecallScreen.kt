...
fun FlashRecallScreen(
    ...
    Column(
        ...
        Text("LocoWiki 极速召回 ${BuildConfig.VERSION_NAME}", style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Bold)
        PrivacyCard(state)
        ContextCard(state.context, onContextChange)
        SpeakerCard(
            state = state,
            onSpeakerModeChange = onSpeakerModeChange,
            onCaptureSelfSample = onCaptureSelfSample,
...
private fun SpeakerCard(
    ...
    SectionCard("说话人识别（可选）") {
        ...
        Text("启动会议模式并说完一段完整句子后，点击采集。共采集3段；只保存本地声纹向量，不保存登记原音频。", style = MaterialTheme.typography.bodySmall)
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(
                onClick = onCaptureSelfSample,
                enabled = profile.modelReady && state.listening,
                modifier = Modifier.weight(1f),
            ) { Text("采集最近SELF语句") }
            OutlinedButton(
                onClick = onDeleteSelfProfile,
                enabled = profile.acceptedSamples > 0 && !state.listening,
                modifier = Modifier.weight(1f),
            ) { Text("删除SELF声纹") }
        }
        ...
        Column(Modifier.selectableGroup()) {
            SpeakerMode.entries.forEach { mode ->
                Row(
                    Modifier.fillMaxWidth().clickable(enabled = !state.listening) { onSpeakerModeChange(mode) },
                    verticalAlignment = Alignment.CenterVertically,
                ) {
                    RadioButton(selected = state.speakerMode == mode, onClick = { onSpeakerModeChange(mode) }, enabled = !state.listening)
        ...
        OutlinedButton(
            ...
            enabled = !state.listening && state.lastSessionSummary != null && !state.speakerReclustering,
            modifier = Modifier.fillMaxWidth(),
    ...
    SectionCard("会议听题与记录") {
        ...
        Column(Modifier.selectableGroup()) {
            RecordMode.entries.forEach { mode ->
                Row(
                    Modifier.fillMaxWidth().clickable(enabled = !state.listening) { onRecordModeChange(mode) },
                    verticalAlignment = Alignment.CenterVertically,
                ) {
                    RadioButton(selected = state.recordMode == mode, onClick = { onRecordModeChange(mode) }, enabled = !state.listening)
                    Text(
                        when (mode) {
                            RecordMode.NONE -> "不保存"
